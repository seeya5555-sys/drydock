...
# ══════════════════════════════════════════════════════════════
# MCP CONNECTOR (Claude 커넥터용 OAuth 2.0 + MCP 엔드포인트)
# ══════════════════════════════════════════════════════════════
import secrets, time
...
def mcp_endpoint():
    ...
    if method == 'tools/list':
        tools = [
            {"name":"get_fleet_summary",  "description":"전체 선단 현황 요약",
             "inputSchema":{"type":"object","properties":{}}},
            {"name":"get_vessels",        "description":"등록된 모든 선박 목록 조회",
             "inputSchema":{"type":"object","properties":{}}},
            {"name":"get_vessel_jobs",    "description":"특정 선박의 작업 목록 조회",
             "inputSchema":{"type":"object","properties":{"vessel_id":{"type":"string","description":"선박 ID"}},"required":["vessel_id"]}},
            {"name":"get_class_items",    "description":"선급 검사 항목 조회",
             "inputSchema":{"type":"object","properties":{"vessel_id":{"type":"string"}},"required":["vessel_id"]}},
        ]
    ...
    if method == 'tools/call':
        ...
        base = request.host_url.rstrip('/')
        # 서비스 계정으로 내부 API 호출 (세션 없이 직접 DB 조회)
        db = get_db()
        if tool == 'get_vessels':
            content = [dict(r) for r in db.execute("SELECT id,name,type,imo FROM vessels").fetchall()]
        elif tool == 'get_vessel_jobs':
            vid = args.get('vessel_id','')
            content = [dict(r) for r in db.execute("SELECT number,section,category,description,status,completion FROM jobs WHERE vessel_id=? ORDER BY number", (vid,)).fetchall()]
        elif tool == 'get_class_items':
            vid = args.get('vessel_id','')
            content = [dict(r) for r in db.execute("SELECT * FROM class_items WHERE vessel_id=?", (vid,)).fetchall()]
        elif tool == 'get_fleet_summary':
            vessels = [dict(r) for r in db.execute("SELECT id,name FROM vessels").fetchall()]
