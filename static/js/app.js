...
function renderJobs(){
  ...
  fil.sort((a,b)=>{let av=a[sKey],bv=b[sKey];if(sKey==='number'){av=pNum(av);bv=pNum(bv);}return av<bv?-sDir:av>bv?sDir:0;});
...
function addInlineRow() {
  ...
  const newJob = {
    ...
    start_date: '', end_date: '', completion: 0, remark: ''
  };
  jobs.push(newJob);
  persist('jobs',jobs);
  buildJFilters();
  renderJobs();
  // Auto-open edit for the new row's description cell
  const newRi = jobs.length - 1;
  requestAnimationFrame(() => {
    const rows = document.querySelectorAll('#j-body tr');
    const lastRow = rows[rows.length - 1];
    if(lastRow) {
      const numCell = lastRow.querySelector('.cell-edit');
      if(numCell) numCell.click();
    }
  });
  toast('새 행이 추가됐습니다. 셀을 클릭해서 바로 입력하세요.');
}
